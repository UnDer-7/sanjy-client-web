 ...
 *
 * <p>This exception is raised when the HTTP client cannot establish a connection to the target service (e.g.,
 * connection refused, DNS resolution failure, timeout). Unlike {@link UnhandledClientHttpException}, which is thrown
 * when the server responds with an HTTP error status (4xx or 5xx), this exception indicates that no HTTP response was
 * received at all.
 *
 * <p>This exception is automatically thrown by the global {@code RestClient} interceptor chain when an
